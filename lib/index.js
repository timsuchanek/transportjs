...
function _receiveChunk(peerID, data, connection) {
	...
	if (!connection.chunking && cb === 'cb') {
		...
		// successfully chunked :-)
			...
			monsole.log('handleMessage with chunked data');

		} else {
			throw new Error('We got more data (' + newLength + ' byte) in the last message than expected (' + chunk.size + ' byte)');
		}



	} else {
		throw new Error("Chunking not possible, got cb=", cb, 'but chunking=', connection.chunking);
	}
